...
"""
Gather results from all the videos while the AP result is incorporated with predicted confidence.
"""
import random
...
class Evaluator(object):
    ...
    def write_report(self, report_file=None):
        ...
        with open(report_file, 'w') as f:
            ...
            f.write('Mean Consistency score: {}\n'.format(
                safe_divide(self._consistency_score, self._num_video)))
    ...
    def update(self, data_json):
        def combine(name, a, b):
            ...
            # New
            for idx, _ in enumerate(a.true_positive):
                for j in b[name]['tp'][idx]:
                    a.true_positive[idx].append(j)
            for idx, _ in enumerate(a.false_positive):
                for j in b[name]['fp'][idx]:
                    a.false_positive[idx].append(j)
...
if __name__ == '__main__':
    ...
    report_folder = 'report/CenterPoseTrack'
                    ...
                    with open(report_file, 'r') as fp:
                        for idx, l in enumerate(fp):
                            if idx == 27:
                                ...
                                consistency_score = line_list[2]  # 2
                                if consistency_score not in ['None', 'nan']:
                                    consistency_score = float(consistency_score)
                                    evaluator._consistency_score += consistency_score
                                    evaluator._num_video += 1
                evaluator.finalize()
