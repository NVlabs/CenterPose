...
class RegKLDScaleLoss(nn.Module):
    def __init__(self):
        super(RegKLDScaleLoss, self).__init__()

    def forward(self, output, uncertainty, mask, ind, target, opt):
        pred = _transpose_and_gather_feat(output, ind)
        ...
        # KLD
        b = torch.ones_like(a) * opt.KL_scale_uncertainty
...
class RegKLDKeyLoss(nn.Module):
    def __init__(self):
        super(RegKLDKeyLoss, self).__init__()
    ...
    def forward(self, output, uncertainty, mask, ind, target, opt):
        ...
        # KLD
        b = torch.ones_like(a) * opt.KL_kps_uncertainty
        var = torch.exp(pred_uncertainty)
        loss = (pred_uncertainty - torch.log(b) + (b * torch.exp(-a / b) + a) / var - 1 + 0.5 * torch.abs(var)) * mask
        loss = loss.sum(dim=(2, 3))
        loss = loss / (mask.sum(dim=(2, 3)) + 1e-6)
